...
This project is an analysis of employee retention and salary prediction. I've sought to encapulsate the ML techniques/powerpoint into the easily repeatable functions(ensembleClassify/ensemblePrediction) so this codebase is portable for future analysis. I've got it about 80% the way there, but ran out of time. In the future I'll clean it up further to remove case-study 2 specific logic.

For continuous dependent variable it will predict using randomforest, lasso, linear_regression,naive bayes, and various combinations of the results of the other models (min, max, median, mean).

For the categorical dependent variable it will predict using randomforest, KNN, Neural Network, Naive Bayes, and by letting those models having an equal-weighted vote.

The functions will rate the performance of each model for classifier accuracy or RMSE. Afterwhich it will annoint a winner and a loser. This and other variables are returned and can be accessed using WINNER_MODEL, WINNER_NAME, LOSER_DESCRIPTION`.
